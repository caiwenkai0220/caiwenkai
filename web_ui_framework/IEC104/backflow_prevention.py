...
class DeviceState:
    def __init__(self):
        ...
        # 设备功率状态：COA -> {功率IOA: 功率值}
        self.device_power: Dict[int, Dict[int, float]] = {
            1: {1: 0.0},  # PCS, COA=1, IOA=1
            2: {1: 0.0},  # 逆变器, COA=2, IOA=1
            3: {1: 0.0}  # 电表, COA=3, IOA=1 (自动计算)
        }
    ...
    def update_device_power(self, coa: int, ioa: int, value: float):
        ...
        with self.lock:
            ...
            if coa in (1, 2):
                ...
                inverter_power = self.device_power[2][1]
                self.device_power[3][1] = pcs_power + inverter_power
                logger.info(
                    f"设备功率更新 - PCS: {pcs_power} | 逆变器: {inverter_power} | 电表: {self.device_power[3][1]}")
...
class IEC104Server:
    ...
    def start_server(self):
        ...
        try:
            ...
            logger.info("模拟设备：PCS(COA=1)、逆变器(COA=2)、电表(COA=3)，功率IOA均为1")
    ...
    def handle_total_interrogation(self, client_socket: socket.socket):
        ...
        # 总召时主动上报所有设备的当前功率
        for coa in [1, 2, 3]:
            power = device_state.get_device_power(coa, 1)
            self.send_yc_float_message(client_socket, power, 1, coa)
            time.sleep(0.1)
    ...
    def handle_float_yt_exec(self, client_socket: socket.socket, data: bytes):
        ...
        # 更新设备功率（仅PCS/逆变器）
        if coa in (1, 2) and ioa_value == 1:
            device_state.update_device_power(coa, ioa_value, float_value)
            # 获取更新后的电表功率
            meter_power = device_state.get_device_power(3, 1)
            logger.info(f"更新设备功率 - COA={coa} IOA={ioa_value}: {float_value}, 电表功率自动更新为: {meter_power}")
        ...
        if coa in (1, 2) and ioa_value ==1:
            ...
            meter_power = device_state.get_device_power(3, 1)
            self.send_yc_float_message(client_socket, meter_power, 1, 3)
        else:
    ...
    def handle_scaled_yt_exec(self, client_socket: socket.socket, data: bytes):
        ...
        # 更新设备功率（仅PCS/逆变器）
        if coa in (1, 2):
            device_state.update_device_power(coa, ioa_value, float(scaled_value))
            # 获取更新后的电表功率
            meter_power = device_state.get_device_power(3, 1)
            logger.info(f"更新设备功率 - COA={coa} IOA={ioa_value}: {scaled_value}, 电表功率自动更新为: {meter_power}")
        ...
        # 3. 上报标度化值（反馈遥调结果）
        # 发送当前设备的最新值
        current_value = int(device_state.get_device_power(coa, ioa_value))
        self.send_yc_scaled_message(client_socket, current_value, ioa_value, coa)

        # 如果是PCS/逆变器，同时上报电表的最新值
        if coa in (1, 2):
            meter_power = device_state.get_device_power(3, 1)
            self.send_yc_scaled_message(client_socket, int(meter_power), 1, 3)
...
if __name__ == "__main__":
    ...
    print("📌 模拟设备信息：")
    print("   - PCS: COA=1, 功率IOA=1")
    print("   - 逆变器: COA=2, 功率IOA=1")
    print("   - 电表: COA=3, 功率IOA=1 (自动计算=PCS+逆变器)")
