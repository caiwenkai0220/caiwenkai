...
import threading
import logging
from typing import Tuple, Optional, List, Dict  # 新增List、Dict类型支持
...
class IEC104Server:
    ...
    # ---------------------- 等待客户端Socket（核心优化） ----------------------
    def wait_client_socket(self, target_ip: str = None, target_port: int = None, timeout: float = 60.0) -> Optional[socket.socket]:
    ...
    # ---------------------- 新增：批量遥信发送 ----------------------
    def send_batch_yx(self,
                      yx_list: List[Dict[str, int]],
                      ca: int = 1,
                      target_ip: str = None,
        ...
        """
        批量发送遥信数据
        :param yx_list: 遥信列表，格式：[{"ioa": 101, "value": 1}, {"ioa": 102, "value": 0}]
        :param ca: 公共地址
        :param target_ip: 目标客户端IP
        :param target_port: 目标客户端端口
        :param timeout: 超时时间
        """
        # 参数校验
        if not yx_list:
            logger.error("批量发送遥信失败：遥信列表为空")
            return
        if len(yx_list) > 0xFF:
            logger.error(f"批量发送遥信失败：最多支持255个点位，当前传入{len(yx_list)}个")
            return

        # 等待客户端连接和总召完成
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
            logger.error("批量发送遥信失败：未获取到客户端Socket")
            return
        if not self.wait_total_interrogation(timeout):
            logger.error("批量发送遥信失败：总召未完成（超时）")
            return

        # 组装并发送批量遥信报文
        self.send_batch_yx_message(client_socket, yx_list, ca)

    def send_batch_yx_message(self, client_socket: socket.socket, yx_list: List[Dict[str, int]], ca: int = 1):
        """批量遥信底层报文组装"""
        num_objects = len(yx_list)
        # 组装ASDU头部：typeid + NumberOfObjects + COT + 预留位 + CA
        asdu_header = struct.pack('<B B B B',
                                   IEC104Const.M_TYPE_YX,
                                   num_objects,
                                   IEC104Const.COT_SPONTANEOUS,
                                   0x00
                                   ) + self._ca_to_bytes(ca)

        # 循环组装每个遥信的IOA(3字节) + 数据(1字节)
        asdu_body = b""
        for idx, yx_item in enumerate(yx_list):
            ioa = yx_item.get("ioa", 0)
            value = yx_item.get("value", 0)
            if ioa <= 0:
                logger.warning(f"跳过无效遥信点位（索引{idx}）：IOA={ioa}")
                continue
            # 遥信值只取1字节（0/1）
            value = value & 0xFF
            asdu_body += self._ioa_to_bytes(ioa) + struct.pack('<B', value)

        asdu = asdu_header + asdu_body
        desc = f"批量遥信(点位数量={num_objects}, IOA列表={[item['ioa'] for item in yx_list]})"
        self.send_i_frame(client_socket, asdu, desc)

    # ---------------------- 新增：批量标度化遥测发送 ----------------------
    def send_batch_yc_scaled(self,
                             yc_scaled_list: List[Dict[str, int]],
                             ca: int = 1,
                             target_ip: str = None,
                             target_port: int = None,
                             timeout: float = 60.0):
        """
        批量发送标度化遥测数据
        :param yc_scaled_list: 标度化遥测列表，格式：[{"ioa": 201, "value": 1234}, {"ioa": 202, "value": 5678}]
        :param ca: 公共地址
        :param target_ip: 目标客户端IP
        :param target_port: 目标客户端端口
        :param timeout: 超时时间
        """
        # 参数校验
        if not yc_scaled_list:
            logger.error("批量发送标度化遥测失败：遥测列表为空")
            return
        if len(yc_scaled_list) > 0xFF:
            logger.error(f"批量发送标度化遥测失败：最多支持255个点位，当前传入{len(yc_scaled_list)}个")
            return

        # 等待客户端连接和总召完成
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
            logger.error("批量发送标度化遥测失败：未获取到客户端Socket")
            return
        if not self.wait_total_interrogation(timeout):
            logger.error("批量发送标度化遥测失败：总召未完成（超时）")
            return

        # 组装并发送批量标度化遥测报文
        self.send_batch_yc_scaled_message(client_socket, yc_scaled_list, ca)

    def send_batch_yc_scaled_message(self, client_socket: socket.socket, yc_scaled_list: List[Dict[str, int]], ca: int = 1):
        """批量标度化遥测底层报文组装"""
        num_objects = len(yc_scaled_list)
        # 组装ASDU头部
        asdu_header = struct.pack('<B B B B',
        ...
        # 循环组装每个标度化遥测的IOA(3字节) + 数据(3字节)
        asdu_body = b""
        for idx, yc_item in enumerate(yc_scaled_list):
            ioa = yc_item.get("ioa", 0)
            value = yc_item.get("value", 0)
            if ioa <= 0:
                logger.warning(f"跳过无效标度化遥测点位（索引{idx}）：IOA={ioa}")
                continue
            # 标度化遥测值取3字节（0~0xFFFFFF）
            value = value & 0xFFFFFF
            scaled_data = struct.pack('<I', value)[:3]  # 取前3字节
            asdu_body += self._ioa_to_bytes(ioa) + scaled_data

        asdu = asdu_header + asdu_body
        desc = f"批量标度化遥测(点位数量={num_objects}, IOA列表={[item['ioa'] for item in yc_scaled_list]})"
        self.send_i_frame(client_socket, asdu, desc)

    # ---------------------- 新增：批量短浮点遥测发送 ----------------------
    def send_batch_yc_float(self,
                           yc_float_list: List[Dict[str, float]],
                           ca: int = 1,
                           target_ip: str = None,
                           target_port: int = None,
                           timeout: float = 60.0):
        """
        批量发送短浮点遥测数据
        :param yc_float_list: 短浮点遥测列表，格式：[{"ioa": 301, "value": 25.8}, {"ioa": 302, "value": 36.9}]
        :param ca: 公共地址
        :param target_ip: 目标客户端IP
        :param target_port: 目标客户端端口
        :param timeout: 超时时间
        """
        # 参数校验
        if not yc_float_list:
            logger.error("批量发送浮点遥测失败：遥测列表为空")
            return
        if len(yc_float_list) > 0xFF:
            logger.error(f"批量发送浮点遥测失败：最多支持255个点位，当前传入{len(yc_float_list)}个")
            return

        # 等待客户端连接和总召完成
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
            logger.error(f"批量发送浮点遥测失败：未获取到客户端Socket")
            return
        if not self.wait_total_interrogation(timeout):
            logger.error(f"批量发送浮点遥测失败：总召未完成（超时）")
            return

        # 组装并发送批量浮点遥测报文
        self.send_batch_yc_float_message(client_socket, yc_float_list, ca)

    def send_batch_yc_float_message(self, client_socket: socket.socket, yc_float_list: List[Dict[str, float]], ca: int = 1):
        """批量短浮点遥测底层报文组装"""
        num_objects = len(yc_float_list)
        # 组装ASDU头部
        asdu_header = struct.pack('<B B B B',
        ...
        # 循环组装每个浮点遥测的IOA(3字节) + 数据(5字节：4字节浮点+1字节预留)
        asdu_body = b""
        for idx, yc_item in enumerate(yc_float_list):
            ioa = int(yc_item.get("ioa", 0))
            value = yc_item.get("value", 0.0)
            if ioa <= 0:
                logger.warning(f"跳过无效遥测点位（索引{idx}）：IOA={ioa}")
                continue
            ioa_bytes = self._ioa_to_bytes(ioa)
            float_bytes = struct.pack('<f B', value, 0x00)  # 浮点+预留位
            asdu_body += ioa_bytes + float_bytes

        asdu = asdu_header + asdu_body
        desc = f"批量浮点遥测(点位数量={num_objects}, IOA列表={[item['ioa'] for item in yc_float_list]})"
        self.send_i_frame(client_socket, asdu, desc)

    # ---------------------- 原有单点位发送方法（保持不变） ----------------------
    def send_yx(self, value: int, ioa: int, ca: int = 1, target_ip: str = None, target_port: int = None,timeout: float = 60.0):
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
    ...
    def send_yc_float(self, value: float, ioa: int, ca: int = 1, target_ip: str = None, target_port: int = None,timeout: float = 60.0):
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
    ...
    def send_yc_scaled(self, value: int, ioa: int, ca: int = 1, target_ip: str = None, target_port: int = None,timeout: float = 60.0):
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
    ...
    def send_ym(self, value: int, ioa: int, ca: int = 1, target_ip: str = None, target_port: int = None,timeout: float = 60.0):
        client_socket = self.wait_client_socket(target_ip, target_port, timeout)
        if not client_socket:
    ...
    def send_yx_message(self, client_socket: socket.socket, yx_value: int, ioa: int, ca: int = 1):
        ...
        self.send_i_frame(client_socket, asdu, f"遥信(IOA=0x{ioa:06X}, 值={yx_value})")
    ...
    def send_yc_float_message(self, client_socket: socket.socket, float_value: float, ioa: int, ca: int = 1):
        ...
        self.send_i_frame(client_socket, asdu, f"短浮点遥测(IOA=0x{ioa:06X}, 值={float_value})")
    ...
    def send_yc_scaled_message(self, client_socket: socket.socket, scaled_value: int, ioa: int, ca: int = 1):
        ...
        self.send_i_frame(client_socket, asdu, f"标度化遥测(IOA=0x{ioa:06X}, 值={scaled_value})")
    ...
    def send_ym_message(self, client_socket: socket.socket, ym_value: int, ioa: int, ca: int = 1):
        ...
        self.send_i_frame(client_socket, asdu, f"遥脉(IOA=0x{ioa:06X}, 值={ym_value})")
    ...
    def handle_yk_select(self,client_socket: socket.socket,data: bytes):
        ...
        self.send_i_frame(client_socket, asdu, f"遥控选择，激活确认")
    ...
    def handle_yk_exec(self, client_socket: socket.socket, data: bytes):
        ...
        # 1. 激活确认报文
        asdu = data[6:8] + cot_activation_byte + data[9:]
        self.send_i_frame(client_socket, asdu, f"遥控激活确认(IOA={ioa_value}, 值={yx_value})")

        # 2. 激活终止报文
        asdu2 = data[6:8] + cot_termination_byte + data[9:]
        self.send_i_frame(client_socket, asdu2, f"遥控激活终止(IOA={ioa_value}, 值={yx_value})")

        # 3. 发送遥信点位（反馈遥控结果）
        self.send_yx(yx_value, ioa_value)
    ...
    def handle_float_yt_select(self, client_socket: socket.socket,data: bytes):
        ...
        # 1. 激活确认报文
        asdu = data[6:8] + cot_activation_byte + data[9:]
        self.send_i_frame(client_socket, asdu, f"浮点遥调选择，激活确认")
    ...
    def handle_float_yt_exec(self, client_socket: socket.socket, data: bytes):
        ...
        # 1. 激活确认报文
        asdu = data[6:8]+cot_activation_byte+data[9:]
        self.send_i_frame(client_socket, asdu, f"短浮点遥调激活确认(IOA={ioa_value}, 值={float_value})")

        # 2. 激活终止报文
        asdu2 = data[6:8] + cot_termination_byte + data[9:]
        self.send_i_frame(client_socket, asdu2, f"短浮点遥调激活终止(IOA={ioa_value}, 值={float_value})")

        # 3. 上报浮点遥测值（反馈遥调结果）
        self.send_yc_float(float_value,ioa_value)
    ...
    def handle_scaled_yt_select(self, client_socket: socket.socket,data: bytes):
        ...
        # 1. 激活确认报文
        asdu = data[6:8] + cot_activation_byte + data[9:]
        self.send_i_frame(client_socket, asdu, f"标度化遥调选择，激活确认")
    ...
    def handle_scaled_yt_exec(self, client_socket: socket.socket,data:bytes):
        ...
        # 1. 激活确认报文
        asdu = data[6:8] + cot_activation_byte + data[9:]
        self.send_i_frame(client_socket, asdu, f"标度化遥调激活确认(IOA={ioa_value}, 值={scaled_value})")

        # 2. 激活终止报文
        asdu2 = data[6:8] + cot_termination_byte + data[9:]
        self.send_i_frame(client_socket, asdu2, f"标度化遥调激活终止(IOA={ioa_value}, 值={scaled_value})")

        # 3. 上报标度化值（反馈遥调结果）
        self.send_yc_scaled(scaled_value, ioa_value)
...
if __name__ == "__main__":
    ...
    def demo_batch_send():
        # 等待总召完成
        if server.wait_total_interrogation(timeout=30):
            # 1. 批量发送遥信
            batch_yx_data = [
                {"ioa": 1, "value": 1},
                {"ioa": 102, "value": 0},
            ...
            server.send_batch_yx(batch_yx_data, target_ip="127.0.0.1")
            ...
            server.send_batch_yc_scaled(batch_scaled_data, target_ip="127.0.0.1")
            time.sleep(3)
            # 3. 批量发送短浮点遥测
            batch_float_data = [
                {"ioa": 11, "value": 25.8},
            ...
            server.send_batch_yc_float(batch_float_data, target_ip="127.0.0.1")

    # 启动批量发送演示线程
    demo_thread = threading.Thread(target=demo_batch_send, daemon=True)
    demo_thread.start()
